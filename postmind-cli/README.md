...
- Make sure the Next.js app is running (`cd reqbeam && npm run dev`) before logging in.

Need to log in without opening a browser (CI, SSH boxes, etc.)? Pass your credentials explicitly:

```bash
pm auth login --email user@example.com --password mypass
```

You can also point to a different API origin:

```bash
pm auth login --api-url https://postmind.internal
