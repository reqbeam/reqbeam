...
- Keep the Next.js app running locally (`cd reqbeam && npm run dev`) while authenticating.
...
```bash
pm auth login --email you@example.com --password superSecret --api-url http://localhost:3000
```
...
### Authentication
```bash
pm auth login      # Opens browser-based login flow
...
```bash
# 1. Authenticate
pm auth login          # opens browser (or pass --email/--password)
...
# Re-login if expired
pm auth logout
pm auth login               # browser flow
# or stay headless
pm auth login --email you@example.com --password superSecret
```
...
```bash
pm auth login        # finish browser login
