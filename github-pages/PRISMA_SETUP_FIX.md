...
Even though `@reqbeam/db` has `@prisma/client` as a dependency, Next.js needs it in the root `node_modules` for proper bundling and to locate the query engine binaries.

**File:** `oss-main/package.json`
```json
"dependencies": {
  "@reqbeam/db": "file:./reqbeam-db",
...
The Prisma client is generated from the shared schema in `reqbeam-db`:
- **Schema:** `oss-main/reqbeam-db/prisma/schema.prisma`
- **Output:** `oss-main/node_modules/@prisma/client` (default location)
- **Command:** `npm run db:generate` (runs in reqbeam-db)
...
1. **Single Schema**: Only `reqbeam-db/prisma/schema.prisma` exists
2. **Single Generated Client**: Generated to web app's `node_modules/@prisma/client`
3. **Shared Instance**: Both web and CLI use the same `prisma` instance from `@reqbeam/db`
...
All commands run in `reqbeam-db` but generate to the web app's `node_modules`.
