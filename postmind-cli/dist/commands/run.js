...
runCommand
    ...
    .action(async (requestName, options) => {
    try {
...
runCommand
    ...
    try {
        ...
        if (options.parallel) {
            ...
            const promises = requests.map(request => RequestExecutor.executeRequest(request, environment || undefined, options.verbose));
...
runCommand
    ...
    .option('-v, --verbose', 'Verbose output')
    .action(async (historyId, options) => {
    try {
        const storage = StorageManager.getInstance();
        const currentProject = await storage.getCurrentProject();
        if (!currentProject) {
