...
## Problem
After consolidating Prisma to use only `@postmind/db`, Next.js couldn't find the Prisma Query Engine:
```
Prisma Client could not locate the Query Engine for runtime "windows"
```

## Root Cause
1. `@postmind/db` had `@prisma/client` as a direct dependency, creating its own instance
...
### 1. Made `@prisma/client` a Peer Dependency in `@postmind/db`
**File:** `oss-main/postmind-db/package.json`
- Removed `@prisma/client` from `dependencies`
- Kept it as `peerDependencies` (required, not optional)
- This ensures `@postmind/db` uses the `@prisma/client` from the root `node_modules`
...
- Changed `db:generate` to run from web app root:
  ```json
  "db:generate": "prisma generate --schema=./postmind-db/prisma/schema.prisma"
  ```
...
**File:** `oss-main/postmind-db/prisma/schema.prisma`
```prisma
generator client {
...
postmind-db/prisma/schema.prisma
    ↓ (prisma generate --schema=./postmind-db/prisma/schema.prisma)
oss-main/node_modules/@prisma/client
    ↓ (peer dependency resolution)
@postmind/db imports '@prisma/client' → resolves to root node_modules
    ↓
...
✅ **FIXED**: 
- `@prisma/client` is now a peer dependency in `@postmind/db`
- Prisma client is generated to web app's `node_modules`
- Both web app and `@postmind/db` use the same `@prisma/client` instance
