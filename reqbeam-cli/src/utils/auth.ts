...
import { UserService } from '@reqbeam/db';
...
export class AuthManager {
  ...
  private constructor() {
    ...
    const configDir = path.join(homeDir, '.reqbeam');
  ...
  public async getAuthHeaders(): Promise<Record<string, string>> {
    ...
    if (!config) {
      throw new Error('Not authenticated. Please run "reqbeam login" first.');
    }
  ...
  public async requireAuth(): Promise<AuthConfig> {
    ...
    if (!config) {
      ...
      console.log(chalk.yellow('Please log in to use Reqbeam CLI:'));
      console.log(chalk.cyan('  reqbeam auth login'));
      console.log(chalk.gray('\nFor more information, see: reqbeam auth --help'));
      process.exit(1);
    }
