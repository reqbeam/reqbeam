# Setup Instructions for @postmind/db Package
...
The web application's `package.json` already includes `@postmind/db` as a local dependency.
...
The CLI's `package.json` already includes `@postmind/db` as a local dependency.
...
import { CollectionService } from '@postmind/db';
...
### Error: Cannot find module '@postmind/db'
...
1. Edit files in `oss-main/postmind-db/src/`
...
1. Create a new service file in `oss-main/postmind-db/src/services/`
2. Export it from `oss-main/postmind-db/src/services/index.ts`
3. Export it from `oss-main/postmind-db/src/index.ts`
4. Rebuild the package

### Updating Prisma Schema

1. Edit `oss-main/postmind-db/prisma/schema.prisma`
...
```
oss-main/
├── postmind-db/          # Shared database package
│   ├── src/
...
│   └── package.json
├── src/                  # Web application
│   └── app/api/          # API routes (use @postmind/db)
└── postmind-cli/         # CLI application
    └── src/
        └── utils/        # Utilities (use @postmind/db)
```
